...
{
    int tope_sup = (m/n)*i; //Calculamos el tope inferior para esta hebra
    int tope_inf = (m/n)*(i-1); //Calculamos el tope superior para esta hebra
    double suma = 0.0 ;                        // inicializar suma
    
    for( long i = tope_inf ; i < tope_sup ; i++ )//La cantidad de puntos donde se calcula f va a ser m/n
      suma += f( (i+double(0.5)) /m );         //   se añade a la suma el valor de f en ese punto
    return suma/m ;          
}
...
{
  //vector de futuros que vamos a tener
  future<double> futuros[n] ;
  //Donde se guarda el valor de Pi  
  double suma_total = 0.0;

  //Ejecutamos las n hebras
  for(int i = 0; i < n; i++){
    futuros[i] = async( launch::async, funcion_hebra, i+1 ) ;
  }

  //Recogemos el resultado dado por las n hebras y lo guardamos
  for(int i = 0; i < n; i++){
    suma_total += futuros[i].get();
  }

  //De volvemos la suma total
  return suma_total;
}
// -----------------------------------------------------------------------------
